...
public class Evenement extends Interaction {
	...
	public Evenement(int id,String nom, String description, int places, int placesRestantes, Date date, int debutH, int debutM, int duree, Utilisateur createur) {
		this(id,nom,description,places,date,debutH,debutM,duree,createur);
	...
	public String toString(){
		...
		return "Événement " + IDENTIFIANT + " : " + nom + " ( " + date + " : " + debutH + " ) ";
	}
	...
	public void afficher() {
		System.out.println("Événement n. " + IDENTIFIANT +" : " + nom);
		System.out.println("Le " + date + " à " + debutH + "h" + debutM + "min ( durée : " + getDureeHM() + " )");
		System.out.println();
		System.out.println("Places : " + places);
		System.out.println("Places restantes : " + placesRestantes);
		System.out.println("Liste principale : ");
		for (Etudiant e : principale) System.out.print(e + " , ");
		System.out.println();
		System.out.println("Liste d'attente : ");
		for (Etudiant e : attente) System.out.print(e + " , ");
		System.out.println();
		System.out.println("--------------------");
	...
	protected boolean push() {
		try{
			Communication com = new Communication(TypeBackupable.EVENEMENT, Action.SAUVEGARDER,this, "Sauvegarde de l'événement " + toString());
	...
	protected void pull() {
		try{
			Communication com = new Communication(TypeBackupable.EVENEMENT, Action.CHARGER,IDENTIFIANT, "Chargement de l'événement " + toString());
			ConnexionServeur.getOOS().writeObject(com);
			Object o =  ConnexionServeur.getIOS().readObject();
			if (!(o instanceof Evenement)) throw new InvalidCommunicationException("La communication n'a pas renvoyé un Evenement", "Événement");
	...
	public static Evenement chercher(int id) {
		try {
			Communication com = new Communication(TypeBackupable.EVENEMENT, Action.CHARGER,id, "Chargement de l'événement " + id);
			ConnexionServeur.getOOS().writeObject(com);
			Object o =  ConnexionServeur.getIOS().readObject();
			if (!(o instanceof Evenement)) throw new InvalidCommunicationException("La communication n'a pas renvoyé un Evenement", "Événement");
			return (Evenement) o;
	...
	public Set<Etudiant> attenteClient(){
		return attente;
	}
	public static Evenement nouveau(String nom, String description, int places, Date date, int debutH, int debutM, int duree, Utilisateur createur) {
		Evenement evt = new Evenement(-1,nom,description,places,date,debutH,debutM,duree,createur);


		try {
			Communication com = new Communication(TypeBackupable.EVENEMENT, Action.NOUVEAU,evt, "Création de l'événement " + evt.toString());
			ConnexionServeur.getOOS().writeObject(com);
			Object o = ConnexionServeur.getIOS().readObject();
			if (!(o instanceof Evenement)) throw new InvalidCommunicationException("La communication n'a pas renvoyé un Evenement", "Événement");
			evt = (Evenement) o;
	...
	public static Set<Integer> ids(){
		...
		try {
			Communication com = new Communication (TypeBackupable.EVENEMENT, Action.GET_IDS, "Récupération des ids des événements");
			ConnexionServeur.getOOS().writeObject(com);
			Object o = ConnexionServeur.getIOS().readObject();
	...
	public static boolean supprimer(int id) {
		boolean reussi = false;
		try {
			Communication com = new Communication(TypeBackupable.EVENEMENT, Action.SUPPRIMER,id, "Suppression de l'événement " + id);
			ConnexionServeur.getOOS().writeObject(com);
			Object o = ConnexionServeur.getIOS().readObject();
	...
	public static Set<Evenement> getAll(){
		...
		try {
			Communication com = new Communication(TypeBackupable.EVENEMENT, Action.GET_ALL, "Récupération de tous les événements");
			ConnexionServeur.getOOS().writeObject(com);
			Object o = ConnexionServeur.getIOS().readObject();
			if ( !(o instanceof Set )) throw new InvalidCommunicationException("La communication n'a pas renvoyé un ensemble");
			Set<?> s = (Set<?>) o;
